...
export default function Home() {
  ...
  const calculateScore = async (formData?: {
    ...
    const finalUrl = formData?.url ?? url;
    const finalName = formData?.name ?? name;
    const finalMobile = formData?.mobile ?? mobile;
    const finalEmail = formData?.email ?? email;

    if (!finalUrl.trim() || !finalName.trim() || !finalMobile.trim() || !finalEmail.trim()) {
      setError("All fields (Name, Mobile, Email, and URL) are required");
      return;
    }

    if (finalMobile.length !== 10 || !/^[6-9]/.test(finalMobile)) {
